						...
						janus.attach(
							{
										...
										if(mystream && mystream.getVideoTracks() && mystream.getVideoTracks().length > 0 && !video) {
											...
											$('#videolocal').append(
												'<div class="no-video-container">' +
													...
													'<span class="no-video-text" style="font-size: 16px;">Video rifiutato, no webcam</span>' +
												'</div>');
										}
									}
								},
								onlocaltrack: function(track, on) {
									...
									if($('#mute').length === 0) {
										...
										$('#videolocal').append('<button type="button" class="btn btn-secondary" id="local-full" style="position: absolute; bottom: 0px; right: 0px; margin: 5px;"> <img src="http://localhost:8000/screen-off.png" alt="Unpublish" style="width:25px;height:25px;"> </img> </button>')
									}
									if(track.kind === "audio") {
