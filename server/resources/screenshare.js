							...
							janus.attach(
								{
									...
									success: function (pluginHandle) {
										...
										//Bottone per selezionare la stanza a cui collegarsi modd
										$('#select').click(SelectRoom);
										//Bottone per richiedere la lista di stanze disponibili
									...
									onmessage: function (msg, jsep) {
										...
										if (event != undefined && event != null) {
											...
											if (event === "joined") {
												...
												// codice che permette di controllare la lista di room disponibili a cui connettermi modd
									...
									onlocaltrack: function (track, on) {
										...
										$('#videos').removeClass('hide');
										if ($('#mute').length === 0) {
											// Add a 'mute' button aggiunte le icone all'interno dei bottoni modd
											$('#videolocal').append('<button type="button" class="btn btn-secondary" id="mute" style="position: absolute; bottom: 0px; left: 0px; margin: 5px;"> <img src="https://localhost/server/unmute.png" alt="Mute" style="width:25px;height:25px;"> </img> </button>');
											$('#mute').click(toggleMute);
											// Add an 'unpublish' button
											$('#videolocal').append('<button type="button" class="btn btn-secondary" id="unpublish" style="position: absolute; bottom: 0px; right: 0px; margin: 5px;"> <img src="https://localhost/server/screen-off.png" alt="Unpublish" style="width:25px;height:25px;"> </img> </button>');
...
// Funzione che permette di mostrare la lista delle room disponibili modd
function listRooms() {

	// Definisco un json lista
	let list = {
		request: "list"
	};
	...
	// Invio della richiesta a Janus per ottenere la lista delle stanze
	sfutest.send({
		message: list,
		
		// Callback che si attiva se arriva una lista non vuota
		success: function (result) {
			...
			for (let i in ids) {
				...
				rooms[i] = ids[i]["room"];
				// Attacco la lista all'elemento HTML
				$('#arr').append($('<option>').val(rooms[i]).text(rooms[i]));
...
// Funzione che permette di selezionare una delle stanze attive modd
function SelectRoom() {
...
// Funzione che permette di creare la room modd
function createRoom() {

	// Prendo la stanza selezionata, faccio il parsing perché di default mi viene restituito un "val"
	let input = $("#create").val();

	// Se non c'è il numero della stanza ti chiede di selezionarlo
	if (input === "") {
		alert("Inserire un numero di stanza")
		return;
	}

	// Recupera il valore del numero della stanza dall'elemento HTML
	...
	// Richiedo a Janus la lista delle stanze
	sfutest.send({
		message: list,

		// Callback: se ci sono stanze le inserisco nella list
		success: function (result) {
			ids = result["list"]
			$('#arr').empty();
			for (let i in ids) {
	...
	// Controllo che la stanza non sia già stata creata
	for (let j in rooms) {
		if (rooms[j] === myroom) {
	...
	// Se tutti i controlli precedenti sono andati
	// a buon fine creo la stanza
	let create = {
		request: "create",
		room: myroom,
		publishers: 6,
	...
	// Invio il messaggio di creazione della stanza a Janus
...
function toggleMute() {
	...
	// Quando si toglie l'audio c'è uno switch dell'icona del microfono modd
	$('#mute').html(muted ? '<img src="https://localhost/server/mute.png" alt="Mute" style="width:25px;height:25px;">' : '<img src="https://localhost/server/unmute.png" alt="Unmute" style="width:25px;height:25px;">');
	...
	janus.attach(
		{
			...
			success: function (pluginHandle) {
				...
				let subscribe = {
					request: "join",
					room: myroom, // questo è l'errore di "feed", quando mi collego ad una stanza che ho creato io il questo join lo faceva nella stanza di default modd
					ptype: "subscriber",
