												...
												if (localVideos === 0) {
													...
													if ($('#videolocal .no-video-container').length === 0) {
														$('#videolocal').append(
															...
															'<span class="no-video-text">Nessuno stream video rilevato</span>' +
															'</div>');
													}
												}
											}
	...
	janus.attach(
		{
			...
			onremotetrack: function (track, mid, on, metadata) {
				...
				if (track.kind === "audio") {
					...
					$('#videoremote' + slot).append(
						'<span class="badge bg-primary hide" id="curres' + slot + '" style="position: absolute; bottom: 0px; left: 0px; margin: 0 0 1px 10px;"></span>' +
						'<span class="badge bg-info hide" id="curbitrate' + slot + '" style="position: absolute; bottom: 0px; right: 0px; margin: 0 0 1px 10px;"></span>');
					Janus.attachMediaStream($('#remotevideo' + slot + '-' + mid).get(0), stream);
